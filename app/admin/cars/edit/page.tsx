...
export default function EditCarPage() {
  ...
  const totalExteriorPhotos = useMemo(
    () => variants.reduce((total, variant) => total + variant.existingExteriorPhotos.length + variant.exteriorPhotos.length, 0),
    [variants],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(`/api/car-detail?id=${parsedId}`, {
          method: "GET", credentials: "same-origin", cache: "no-store", signal: controller.signal,
          headers: { Accept: "application/json" },
        });
        const data = (await response.json().catch(() => null)) as CarDetailResponse | null;
        if (response.status === 401) { location.replace("/admin/login/"); return; }
        if (!response.ok || !data?.success || !data.car) throw new Error(data?.error || "Не удалось загрузить автомобиль.");
        const car = data.car;
        setForm({
          brand: car.brand, model: normalizeUpper(car.model, 100), year: car.year != null ? String(car.year) : "2026", trim: car.trim || "",
          status: car.status, countryCode: car.countryCode || "KR", arrivalDate: car.arrivalDate || "", isNew: car.isNew, mileageKm: String(car.mileageKm ?? 0),
          engineText: car.engineText || "", engineDisplacementL: car.engineDisplacementL != null ? String(car.engineDisplacementL) : "", fuelType: car.fuelType || "",
          driveType: car.driveType || "", transmission: car.transmission || "automatic", seats: car.seats != null ? String(car.seats) : "",
          horsepowerHp: car.horsepowerHp != null ? String(car.horsepowerHp) : "", torqueNm: car.torqueNm != null ? String(car.torqueNm) : "",
          acceleration0100: car.acceleration0100 != null ? String(car.acceleration0100) : "", topSpeedKmh: car.topSpeedKmh != null ? String(car.topSpeedKmh) : "",
          fuelConsumptionL100: car.fuelConsumptionL100 != null ? String(car.fuelConsumptionL100) : "", electricRangeKm: car.electricRangeKm != null ? String(car.electricRangeKm) : "",
          price: car.price != null ? String(car.price) : "", currency: car.currency || "USD", priceOnRequest: car.priceOnRequest, instagramUrl: car.instagramUrl || "",
          shortDescriptionRu: car.shortDescriptionRu || "", shortDescriptionUz: car.shortDescriptionUz || "", descriptionRu: car.descriptionRu || "", descriptionUz: car.descriptionUz || "",
  ...
  async function deleteExistingPhoto(localId: string, group: PhotoGroup, photoId: number) {
    if (!window.confirm(language === "uz" ? "Surat o‘chirilsinmi?" : "Удалить фотографию?")) return;
    setError(null);
    try {
      const response = await fetch(`/api/car-media?id=${photoId}`, { method: "DELETE", credentials: "same-origin", cache: "no-store", headers: { Accept: "application/json" } });
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!carId) { setError("Не удалось определить ID автомобиля."); return; }
    const validationError = validate();
    if (validationError) { setError(validationError); errorRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); return; }

    setSaving(true); setSavingText(t("Сохраняем данные…")); setError(null);
    try {
      const requestedPublic = form.isPublic;
      const hasExistingExterior = variants.some((variant) => variant.existingExteriorPhotos.length > 0);
      const hasNewExterior = variants.some((variant) => variant.exteriorPhotos.length > 0);
      const publishDuringPatch = requestedPublic && hasExistingExterior;
      const payload = {
        id: carId, brand: form.brand, model: form.model.trim(), year: Number(form.year), trim: form.trim.trim() || null,
        status: form.status, countryCode: form.countryCode, arrivalDate: showArrivalDate ? (form.arrivalDate || null) : null,
        isNew: form.isNew, mileageKm: form.isNew ? 0 : Number(form.mileageKm || "0"),
        engineText: form.engineText.trim() || null, engineDisplacementL: form.engineDisplacementL || null, fuelType: form.fuelType || null,
        driveType: form.driveType || null, transmission: form.transmission || "automatic", seats: form.seats || null,
        horsepowerHp: form.horsepowerHp || null, torqueNm: form.torqueNm || null, acceleration0100: form.acceleration0100 || null,
        topSpeedKmh: form.topSpeedKmh || null, fuelConsumptionL100: form.fuelConsumptionL100 || null, electricRangeKm: form.electricRangeKm || null,
        price: form.priceOnRequest ? null : (form.price || null), currency: form.currency, priceOnRequest: form.priceOnRequest, instagramUrl: form.instagramUrl.trim() || null,
        shortDescriptionRu: form.shortDescriptionRu.trim() || null, shortDescriptionUz: form.shortDescriptionUz.trim() || null,
        descriptionRu: form.descriptionRu.trim() || null, descriptionUz: form.descriptionUz.trim() || null, isPublic: publishDuringPatch, isFeatured: form.isFeatured,
        variants: variants.map((variant) => ({ id: variant.dbId, exteriorColorName: variant.exteriorColorName.trim() || null, exteriorSwatch: variant.exteriorSwatch,
          interiorColorName: variant.interiorColorName.trim() || null, interiorSwatch: variant.interiorSwatch, vin: variant.vin.trim().toUpperCase() || null,
          stockNumber: variant.stockNumber.trim().toUpperCase() || null, quantity: Number(variant.quantity || "1") })),
      };
      const response = await fetch("/api/car-detail", { method: "PATCH", credentials: "same-origin", cache: "no-store", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const data = (await response.json().catch(() => null)) as UpdateCarResponse | null;
      if (response.status === 401) { location.replace("/admin/login/"); return; }
      if (!response.ok || !data?.success) throw new Error(data?.error || "Не удалось сохранить изменения.");
      const savedVariants = data.variants ?? [];
      if (savedVariants.length !== variants.length) throw new Error("D1 не подтвердил все цветовые варианты.");

      let uploaded = 0; const total = totalPhotos;
      for (let index = 0; index < variants.length; index += 1) {
        const draft = variants[index]; const dbVariant = savedVariants.find((item) => item.index === index);
        ...
        for (let photoIndex = 0; photoIndex < draft.exteriorPhotos.length; photoIndex += 1) {
          ...
          await uploadPhoto({ carId, variantId: dbVariant.id, group: "exterior", photo: draft.exteriorPhotos[photoIndex], sortOrder: draft.existingExteriorPhotos.length + photoIndex, isCover: !hasExistingExterior && index === 0 && photoIndex === 0 });
          uploaded += 1;
        }
      ...
      if (requestedPublic && !publishDuringPatch && hasNewExterior) await publishCar(carId);
      setCreatedCar({ id: carId, title: `${form.brand} ${form.model}`.trim() });
      window.setTimeout(() => location.assign(`/admin/cars/?created=${carId}`), 850);
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : "Не удалось сохранить изменения.");
    } finally { setSaving(false); setSavingText(null); }
  }

  if ((!authReady || detailLoading) && !error) {
    return (
    ...
    <main className={styles.page} data-theme={theme}>
      ...
      <div className={styles.shell}>
        <section className={styles.intro}>
          ...
          <h1>{language === "uz" ? "Avtomobilni tahrirlash" : "Редактирование автомобиля"}</h1>
          <p className={styles.introText}>{language === "uz" ? "D1 va R2 dagi mavjud kartochka ma’lumotlarini yangilang." : "Измените данные существующей карточки в D1 и R2."}</p>
        </section>
        ...
        <form className={styles.form} onSubmit={submit} noValidate>
          ...
          <div className={styles.saveDock}>
            <div className={styles.saveMeta}>
              ...
              <strong>{form.model || (language === "uz" ? "AVTOMOBIL" : "АВТОМОБИЛЬ")}</strong>
            </div>
            <button className={styles.saveButton} type="submit" disabled={saving || !authReady}>
              ...
              <span>{saving ? (savingText || t("Сохраняем данные…")) : (language === "uz" ? "O‘zgarishlarni saqlash" : "Сохранить изменения")}</span>
            </button>
          </div>
        </form>
      </div>
      ...
      {createdCar ? (
        <div className={styles.successOverlay} role="status" aria-live="polite">
          <div className={styles.successCard}>
            ...
            <p>{language === "uz" ? "O‘zgarishlar saqlandi" : "Изменения сохранены"}</p>
